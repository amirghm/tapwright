...
# Also adds idempotent instruction/adaptor blocks so agents can use `@mobile`.
set -euo pipefail
...
cp -R "$PACK/skills/."    "$AGENT_DIR/skills/"
...
Use tapwright when the user asks for \`@mobile\`, mobile app inspection, mobile automation, manual UI testing, mobile debugging, E2E execution, recording, replay, or screenshot/design comparison.

- Primary entrypoint: \`@mobile\`.
...
install_agent_adapters() {
  ...
  if [[ "$AGENT_DIR" == ".claude" || -d .claude ]]; then
    ...
    write_file_if_missing_or_tapwright_owned \
      ...
      "tapwright:generated" \
      "---"$'\n'"description: tapwright @mobile for Android/iOS inspection, automation, manual testing, debugging, recording, replay, compare, and E2E."$'\n'"---"$'\n\n'"<!-- tapwright:generated -->"$'\n\n'"Read \`$AGENT_DIR/skills/mobile/SKILL.md\` and follow \`$AGENT_DIR/workflows/mobile.md\`. Treat the rest of the prompt as the @mobile request."
  fi
  ...
  if [[ "$AGENT_DIR" == ".cursor" || -d .cursor ]]; then
    ...
    write_file_if_missing_or_tapwright_owned \
      ...
      "tapwright:generated" \
      "---"$'\n'"description: Use tapwright @mobile for mobile app inspection, automation, manual testing, debugging, recording, replay, compare, and E2E."$'\n'"alwaysApply: false"$'\n'"---"$'\n\n'"<!-- tapwright:generated -->"$'\n\n'"When the user mentions @mobile or asks to control/test a mobile app, read \`$AGENT_DIR/skills/mobile/SKILL.md\` and follow \`$AGENT_DIR/workflows/mobile.md\`. Prefer emulators/simulators and ask before touching physical devices."
  fi
  ...
  if [[ -d .opencode ]]; then
    ...
    write_file_if_missing_or_tapwright_owned \
      ...
      "tapwright:generated" \
      "---"$'\n'"description: tapwright @mobile agent for Android/iOS app inspection, automation, manual testing, debugging, recording, replay, compare, and E2E."$'\n'"---"$'\n\n'"<!-- tapwright:generated -->"$'\n\n'"You are @mobile. Read \`$AGENT_DIR/skills/mobile/SKILL.md\`, then use the tapwright workflows and device skills installed under \`$AGENT_DIR\`. Prefer emulators/simulators; ask before touching physical devices."
  fi
...
For mobile app inspection, automation, debugging, manual UI testing, record/replay, compare, or E2E requests, use the tapwright pack installed under \`$AGENT_DIR\`. Read \`$AGENT_DIR/skills/mobile/SKILL.md\` first. Prefer emulators/simulators and ask before interacting with physical devices.
